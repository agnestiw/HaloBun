  ...
  if (empty($latestArticles)) {
    $latestArticles = [
      [
        ...
        'excerpt' => 'Kenali tanda awal kehamilan, cek nutrisi harian, dan tips menghadapi mual agar tetap bugar.',
      ...
      [
        ...
        'excerpt' => 'Contoh menu harian, porsi ideal, dan cara memenuhi kebutuhan zat besi serta asam folat.',
        'date' => now()->subDays(3)->format('d M Y'),
        'read_time' => '7 menit baca',
        'url' => url('/artikel/nutrisi-ibu-hamil'),
        'thumbnail' => 'https://images.unsplash.com/photo-1484980972926-edee96e0960d?q=80&w=1200&auto=format&fit=crop'
      ],
      [
        'title' => 'Olahraga Ringan yang Aman Saat Hamil',
        'excerpt' => 'Rekomendasi peregangan dan jalan kaki 30 menit untuk menjaga kebugaran selama kehamilan.',
  ...
  if (empty($latestFaqs)) {
    $latestFaqs = [
      [
        ...
        'answer' => 'Boleh dalam jumlah terbatas (sekitar 1 cangkir/hari). Konsultasikan dengan tenaga kesehatan jika memiliki kondisi tertentu.'
      ],
      [
        'question' => 'Kapan pertama kali saya perlu USG?',
        'answer' => 'Biasanya pada trimester pertama (sekitar minggu ke-8–12) untuk memastikan perkembangan janin dan usia kehamilan.'
      ],
      [
        'question' => 'Bagaimana cara mengatasi kram kaki di malam hari?',
        'answer' => 'Cobalah peregangan ringan sebelum tidur, jaga hidrasi, dan penuhi kebutuhan kalsium serta magnesium.'
      ],
    ];
  }
    ...
    <div class="grid md:grid-cols-2 gap-8 items-center">
      <div>
        ...
        <p class="mt-4 text-gray-600 leading-relaxed">
          Informasi kesehatan terpercaya untuk ibu hamil: panduan mingguan, artikel, video edukasi, dan jawaban FAQ—
      ...
      <div class="soft-card rounded-2xl p-6 border border-pink-100">
        <div class="flex items-start gap-4">
          <div class="w-12 h-12 rounded-full bg-gradient-to-br from-pink-400 to-purple-400 flex items-center justify-center">
            <svg class="w-6 h-6 text-white" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              ...
              <path d="M8 19a4 4 0 008 0H8z"/>
            </svg>
          </div>
          <div>
            <h2 class="text-xl font-semibold text-gray-900">Tenang, Bun. Kami di sini.</h2>
            <p class="mt-2 text-gray-600 text-sm leading-relaxed">
              Dapatkan ringkasan terbaru seputar kehamilan, nutrisi, dan tips harian. Konten terstruktur, mudah dipahami.
            </p>
            <ul class="mt-3 text-sm text-gray-700 space-y-1">
              <li class="flex items-center gap-2">
                <span class="w-1.5 h-1.5 rounded-full bg-pink-400"></span> Informasi valid dari sumber tepercaya
              </li>
              <li class="flex items-center gap-2">
                <span class="w-1.5 h-1.5 rounded-full bg-pink-400"></span> Desain bersih dan nyaman dilihat
              </li>
              <li class="flex items-center gap-2">
                <span class="w-1.5 h-1.5 rounded-full bg-pink-400"></span> Bisa diakses di semua perangkat
              </li>
            </ul>
          </div>
        </div>
    ...
    <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
      @php
        $menus = [
          ...
          ['label'=>'Fasilitas','url'=>url('/fasilitas'),'icon'=> 'M4 4h16v6H4zM4 14h7v6H4zM13 14h7v6h-7z'],
          ['label'=>'Konsultasi','url'=>url('/konsultasi'),'icon'=> 'M12 20l9-5-9-5-9 5 9 5z M12 12V4'],
          ['label'=>'FAQ','url'=>url('/faq'),'icon'=> 'M8 10h8M8 14h5 M12 19a7 7 0 100-14 7 7 0 000 14z'],
          ['label'=>'Kontak','url'=>url('/contact'),'icon'=> 'M21 8l-9 6-9-6 M3 19h18'],
        ];
      @endphp
      @foreach ($menus as $m)
        <a href="{{ $m['url'] }}" class="group rounded-xl border border-pink-100 bg-white hover:bg-pink-50/50 transition p-4 flex flex-col items-center text-center">
          <div class="w-10 h-10 rounded-full bg-pink-100 flex items-center justify-center group-hover:scale-105 transition">
...
<section aria-labelledby="latest" class="mt-12 sm:mt-14">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex items-center justify-between">
      <h2 id="latest" class="text-xl font-semibold text-gray-900">Terbaru untuk Ibu</h2>
      <div class="hidden sm:flex items-center gap-3 text-sm">
        <a href="{{ url('/artikel') }}" class="text-pink-600 hover:underline">Semua Artikel</a>
        <span class="text-gray-300">•</span>
        <a href="{{ url('/video') }}" class="text-pink-600 hover:underline">Semua Video</a>
        <span class="text-gray-300">•</span>
        <a href="{{ url('/faq') }}" class="text-pink-600 hover:underline">Semua FAQ</a>
      </div>
    </div>
    ...
    <div class="mt-6">
      ...
      <div class="mt-3">
        <a href="{{ url('/artikel') }}" class="inline-flex items-center text-sm text-pink-600 hover:underline">
          Lihat semua artikel
        </a>
      </div>
    </div>
    ...
    <div class="mt-10">
          ...
          <a href="{{ $v['url'] ?? '#' }}" aria-label="Tonton video: {{ $v['title'] ?? 'Video' }}"
            ...
            <div class="relative aspect-video bg-pink-50">
              @if(!empty($v['thumbnail']))
                <img src="{{ $v['thumbnail'] }}" alt="Thumbnail {{ $v['title'] ?? 'Video' }}" class="w-full h-full object-cover">
              @else
                <div class="absolute inset-0 flex items-center justify-center">
                  <svg class="w-10 h-10 text-pink-400" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                    <path d="M8 5v14l11-7-11-7z"/>
                  </svg>
                </div>
              @endif
      ...
      <div class="mt-3">
        <a href="{{ url('/video') }}" class="inline-flex items-center text-sm text-pink-600 hover:underline">
          Lihat semua video
        </a>
      </div>
    </div>
    ...
    <div class="mt-10">
      <div class="flex items-center gap-2">
        <span class="w-8 h-8 rounded-full bg-pink-100 flex items-center justify-center">
          <svg class="w-4 h-4 text-pink-600" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M12 2a10 10 0 100 20 10 10 0 000-20zm1 15h-2v-2h2v2zm1.07-7.75l-.9.92A3.5 3.5 0 0011 14h-2v-.5a5.5 5.5 0 011.61-3.9l1.2-1.2a1.5 1.5 0 10-2.12-2.12l-.69.7-1.42-1.42.7-.7a3.5 3.5 0 014.95 4.95z"/>
          </svg>
        </span>
          ...
          <div class="rounded-2xl border border-pink-100 bg-white p-4">
            <details class="group">
              <summary class="list-none cursor-pointer flex items-center justify-between gap-3">
                ...
                <svg class="w-4 h-4 text-pink-600 group-open:rotate-180 transition" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      ...
      <div class="mt-3">
        <a href="{{ url('/faq') }}" class="inline-flex items-center text-sm text-pink-600 hover:underline">
          Lihat semua FAQ
        </a>
      </div>
    </div>
...
{{-- Fasilitas Kesehatan --}}
<section aria-labelledby="facilities" class="mt-12 sm:mt-14">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex items-center justify-between">
      <h2 id="facilities" class="text-xl font-semibold text-gray-900">Fasilitas Kesehatan</h2>
      <a href="{{ url('/fasilitas') }}" class="hidden sm:inline-flex items-center text-sm text-pink-600 hover:underline">
    ...
    <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      @forelse($facList as $f)
        <div class="rounded-2xl border border-pink-100 bg-white hover:shadow-md transition overflow-hidden">
          <div class="p-4 flex items-start gap-3">
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center flex-wrap gap-2">
                <h3 class="font-semibold text-gray-900 truncate">{{ $f['name'] ?? 'Nama Fasilitas' }}</h3>
                @if(!empty($f['type']))
                ...
                @endif
                @if(isset($f['open_now']))
                  <span class="text-xs px-2 py-0.5 rounded-full {{ $f['open_now'] ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700' }}">
              ...
              <div class="mt-3 flex items-center gap-3">
                @if(!empty($f['url']))
                  <a href="{{ $f['url'] }}" class="text-sm text-pink-600 hover:underline">Detail</a>
                @endif
                @if(!empty($f['map_url']))
                  <a href="{{ $f['map_url'] }}" class="text-sm text-pink-600 hover:underline">Lihat Peta</a>
                @endif
              </div>
            </div>
          </div>
    ...
    <div class="mt-3 sm:hidden">
      <a href="{{ url('/fasilitas') }}" class="inline-flex items-center text-sm text-pink-600 hover:underline">
        Lihat semua fasilitas
      </a>
    </div>
  </div>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="rounded-2xl border border-pink-100 bg-gradient-to-r from-pink-50 to-purple-50 p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-gray-700 text-sm">Kunjungi halaman Konsultasi untuk terhubung dengan tenaga kesehatan.</p>
      </div>
