@extends('layouts.admin')

@section('title', 'Manajemen Artikel - HaloBun')

@section('content')
@php
  $articles = $articles ?? [
    ['id'=>1,'title'=>'Perkembangan Janin Minggu 8','category'=>'Perkembangan Janin','trimester'=>'1','status'=>'Published','updated_at'=>'2025-09-10'],
    ['id'=>2,'title'=>'Panduan Nutrisi Trimester 2','category'=>'Nutrisi','trimester'=>'2','status'=>'Draft','updated_at'=>'2025-09-08'],
    ['id'=>3,'title'=>'Latihan Pernapasan Menjelang Persalinan','category'=>'Persalinan','trimester'=>'3','status'=>'Published','updated_at'=>'2025-09-06'],
  ];
  $categories = $categories ?? ['Semua','Perkembangan Janin','Nutrisi','Persalinan'];
  $statuses = $statuses ?? ['Semua','Published','Draft'];
  $trimesters = ['Semua','1','2','3'];
@endphp

<header class="mb-6">
  <h1 class="text-2xl md:text-3xl font-bold text-balance">
    <span class="bg-clip-text text-transparent bg-gradient-to-r from-pink-500 to-purple-600">
      Manajemen Artikel
    </span>
  </h1>
  <p class="mt-1 text-sm text-gray-600">Cari, filter, tambah, dan kelola artikel.</p>
</header>

<div class="rounded-2xl border border-pink-100 bg-white/80 p-4 mb-4">
  <form class="grid gap-3 md:grid-cols-4" onsubmit="return false;">
    <label class="flex items-center gap-2 md:col-span-2">
      <span class="sr-only">Pencarian</span>
      <input id="search-articles" type="search" placeholder="Cari judul atau isi..."
             class="w-full rounded-xl border border-pink-100 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-pink-300" />
    </label>
    <label class="flex items-center gap-2">
      <span class="sr-only">Kategori</span>
      <select id="filter-category" class="w-full rounded-xl border border-pink-100 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-pink-300">
        @foreach ($categories as $c)
          <option value="{{ $c }}">{{ $c }}</option>
        @endforeach
      </select>
    </label>
    <label class="flex items-center gap-2">
      <span class="sr-only">Trimester</span>
      <select id="filter-trimester" class="w-full rounded-xl border border-pink-100 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-pink-300">
        @foreach ($trimesters as $t)
          <option value="{{ $t }}">{{ $t }}</option>
        @endforeach
      </select>
    </label>
    <label class="flex items-center gap-2">
      <span class="sr-only">Status</span>
      <select id="filter-status" class="w-full rounded-xl border border-pink-100 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-pink-300">
        @foreach ($statuses as $s)
          <option value="{{ $s }}">{{ $s }}</option>
        @endforeach
      </select>
    </label>
    <div class="md:col-span-1 flex items-center gap-3">
      <button type="button"
        class="inline-flex items-center rounded-full bg-gradient-to-r from-pink-500 to-purple-600 text-white px-4 py-2 text-sm font-medium shadow hover:opacity-95 focus:outline-none focus:ring-2 focus:ring-pink-300">
        + Tambah Artikel
      </button>
      <button type="button" id="reset-articles"
        class="rounded-full border border-pink-200 text-pink-700 px-4 py-2 text-sm hover:bg-pink-50">
        Reset
      </button>
    </div>
  </form>
</div>

<div class="overflow-x-auto rounded-2xl border border-pink-100 bg-white/80">
  <table class="min-w-full text-sm">
    <thead class="text-left text-gray-600">
      <tr class="border-b border-pink-100">
        <th class="px-4 py-3">Judul</th>
        <th class="px-4 py-3">Kategori</th>
        <th class="px-4 py-3">Trimester</th>
        <th class="px-4 py-3">Status</th>
        <th class="px-4 py-3">Diubah</th>
        <th class="px-4 py-3">Aksi</th>
      </tr>
    </thead>
    <tbody id="articles-body">
      @foreach ($articles as $a)
        <tr class="border-b border-pink-50 hover:bg-pink-50/30"
            data-title="{{ Str::lower($a['title']) }}"
            data-category="{{ $a['category'] }}"
            data-trimester="{{ $a['trimester'] }}"
            data-status="{{ $a['status'] }}">
          <td class="px-4 py-3 font-medium text-gray-900">{{ $a['title'] }}</td>
          <td class="px-4 py-3">{{ $a['category'] }}</td>
          <td class="px-4 py-3">{{ $a['trimester'] }}</td>
          <td class="px-4 py-3">
            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs
              @if($a['status'] === 'Published') bg-green-50 text-green-700 border border-green-200
              @else bg-yellow-50 text-yellow-700 border border-yellow-200 @endif">
              {{ $a['status'] }}
            </span>
          </td>
          <td class="px-4 py-3 text-gray-600">{{ $a['updated_at'] }}</td>
          <td class="px-4 py-3">
            <div class="flex gap-2">
              <button class="px-3 py-1 rounded-full border border-pink-200 text-pink-700 hover:bg-pink-50 text-xs">Edit</button>
              <button class="px-3 py-1 rounded-full bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 text-xs">Hapus</button>
            </div>
          </td>
        </tr>
      @endforeach
    </tbody>
  </table>
</div>
@endsection

@push('scripts')
<script>
  (function(){
    const search = document.getElementById('search-articles');
    const cat = document.getElementById('filter-category');
    const tri = document.getElementById('filter-trimester');
    const stat = document.getElementById('filter-status');
    const reset = document.getElementById('reset-articles');
    const rows = Array.from(document.querySelectorAll('#articles-body tr'));

    function apply() {
      const q = (search.value || '').trim().toLowerCase();
      const c = cat.value;
      const t = tri.value;
      const s = stat.value;
      rows.forEach(r => {
        const hit =
          ((q === '') || r.dataset.title.includes(q)) &&
          ((c === 'Semua') || r.dataset.category === c) &&
          ((t === 'Semua') || r.dataset.trimester === t) &&
          ((s === 'Semua') || r.dataset.status === s);
        r.style.display = hit ? '' : 'none';
      });
    }

    [search, cat, tri, stat].forEach(el => el && el.addEventListener('input', apply));
    [cat, tri, stat].forEach(el => el && el.addEventListener('change', apply));
    reset && reset.addEventListener('click', () => {
      search.value = '';
      cat.selectedIndex = 0;
      tri.selectedIndex = 0;
      stat.selectedIndex = 0;
      apply();
    });
    apply();
  })();
</script>
@endpush
